...
- **git 独立 clone**：确认 origin 指向官方仓 https://github.com/ruodou233/improve-product-plan 后 `git fetch`；本地 HEAD 与 `origin/main` 相等则无更新；仅当本地 HEAD 落后且可 fast-forward 到 `origin/main` 时视为有更新。本地领先、分叉或工作树有改动时不提示更新，只提醒使用者本地有未同步改动。
